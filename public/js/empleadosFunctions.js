...
funcionE.empleadosAccessAll = (acc, sign, callback) => {

    dbE.query(`SELECT acc_id FROM del_accesos WHERE acc_embarques ${sign}${acc}`, function (err, result, fields) {
        if (err) {
            callback(err, null);
        } else {
...
funcionE.empleadosAccesos = (callback) => {
    ...
    dbE.query(`SELECT acc_id, acc_embarques FROM del_accesos WHERE acc_embarques>0`, function (err, result, fields) {
        if (err) {
            callback(err, null);
        } else {
...
funcionE.empleadosDeleteAcceso = (acc_id, callback) => {
    ...
    dbE.query(`UPDATE del_accesos SET acc_embarques=0 WHERE acc_id=${acc_id}`, function (err, result, fields) {
            if (err) {
...
funcionE.empleadosInsertAcceso = (acc_id, acc_em, callback) => {

    dbE.query(`INSERT INTO del_accesos (acc_id, acc_embarques)
    VALUES('${acc_id}',${acc_em})
    ON DUPLICATE KEY UPDATE acc_embarques= VALUES(acc_embarques)`, function (err, result, fields) {
            if (err) {
